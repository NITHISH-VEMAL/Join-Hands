...
<sql:query var="rs" dataSource="${db}"> select * from service where email = '${email}'</sql:query>
...
<div class="card">
  ...
  <br/><br/>
    ...
    <c:set var="nameservice" value="${row.name}" scope="session" />
...
<sql:query var="rs" dataSource="${db}"> select * from ${nameservice}</sql:query>
...
<div class="card">
  ...
  <div class="container">
  <br/><br/>
<h3>Customers in-need of your service</h3>
	<br/><br/><br/>
	    
    <c:forEach items="${rs.rows}" var="row">
    
	<h4><c:out value="${row.username}" ></c:out></h4> <button style="background-color:black; color:white; margin-left:0%;"  onclick="this.innerHTML='Accepted'"   >Accept</button>
