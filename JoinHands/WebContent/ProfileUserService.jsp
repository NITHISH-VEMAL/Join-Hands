<%@ page import="java.io.*,java.util.*,java.sql.*,javax.servlet.http.HttpSession, javax.servlet.http.HttpServletRequest"%>  
...
<%String sql = request.getParameter("reqname"); 

HttpSession sess=request.getSession();
sess.setAttribute("profile",sql);
...
<sql:query var="rs" dataSource="${db}"> select * from service where name = '${profile}' </sql:query>
...
<div class="card">
  ...
  <br/><br/>
    ...
    <p style="font-size:15px;">Profile of the selected professional</p><br/><br/><br/><br/>
	...
	<h6> Service location </h6> <h4><c:out value="${row.area}" > </c:out> </h4> <h4>, <c:out value="${row.city}" > </c:out> </h4> <br/><br/>
	 <h6> Experience </h6><h4> <c:out value="${row.experience}" > </c:out> </h4> <br/><br/>
	<h6>Contact number </h6><h4><c:out value="${row.phonenumber}" ></c:out></h4> <br/><br/>
	<h6>Email </h6><h4><c:out value="${row.email}" ></c:out></h4> <br/><br/>
	<br/><br/>
	
	
	<button style="background-color:black; color:white; margin-left:0%;"  onclick="this.innerHTML='Requested'"  onclick="insert()" >Request</button>
...
function insert(){
		...
		<sql:update var="rs" dataSource="${db}"> insert into ${profile} values ('${email}') </sql:update>
